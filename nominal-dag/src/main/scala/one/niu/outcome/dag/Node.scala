...
class Node(val isRoot: Boolean = true, val isLeaf:Boolean = false, val level: Int, val symbol: ByteBuffer){
  ...
  val keyDictionary : mutable.MultiMap[ByteBuffer,ByteBuffer] = new mutable.HashMap[ByteBuffer,mutable.Set[ByteBuffer]]() with mutable.MultiMap[ByteBuffer,ByteBuffer]
